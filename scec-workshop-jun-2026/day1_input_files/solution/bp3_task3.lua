...
BP3.b = 0.015       -- Rate-and-state evolution effect parameter, b
...
-- Task 3: heterogeneous elastic structure across the fault [GPa]
BP3.mu_overriding  = 30.0   -- shear modulus of the overriding (hanging-wall) plate
BP3.mu_subducting  = 25.0   -- shear modulus of the subducting (foot-wall) plate
...
-- Shear modulus [GPa]
-- Task 3: assign a different shear modulus to each plate. The fault plane passes
-- through the origin with the given dip; dist > 0 selects the overriding
-- (hanging-wall) side, dist < 0 the subducting (foot-wall) side.
function BP3:mu(x, y)
    local dist = x + y / math.tan(self.dip)
    if dist > 0 then
        return self.mu_overriding
    else
        return self.mu_subducting
    end
end
