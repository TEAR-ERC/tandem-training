...
function BP3:shearVel(x, y)
    -- Use exponential function for depth-varying property
    return self:exp_fun(x, y, 1.2, 4.0, 5.0)
end

-- Density [g/cm3]; to define material property
function BP3:density(x, y)
    -- Use exponential function for depth-varying property
    return self:exp_fun(x, y, 2.2, 2.9, 5.0)
end

-- Exponential function to define depth-varying property
function BP3:exp_fun(x, y, surface_value, mantle_vaule, lambda_val)
    local depth = math.abs(y)
    -- Exponential formula
    return surface_value + (mantle_vaule - surface_value) * (1 - math.exp(-depth / lambda_val))
end
