...
def detect_events(outputs, dep, options):
    """
    Detect seismic events from Tandem fault-probe outputs.
    ...
    --------
    event_info : dict
        Earthquake information dictionary with event times, indices, depths, slip, rupture properties, clusters, and detection options.
    """
    options['SSE'] = False
...
def detect_SSEs(outputs, dep, options):
    """
    Detect slow-slip events from Tandem fault-probe outputs.
    ...
    --------
    SSE_info : dict
        Slow-slip event information dictionary with event times, indices, depths, slip, rupture properties, and detection options.
    """
    options['SSE'] = True
    tstart, tend, evdep, its_all, ite_all, Vmax, duration = event_times(outputs, dep, options)
...
def event_times(outputs, dep, options):
    """
    Find event start times, end times, depths, and peak slip rates.
    ...
    Returns:
    --------
    tstart, tend, evdep, its_all, ite_all, Vmax, duration : tuple
        Event start times, end times, event depths, start indices, end indices, peak slip rates, and durations.
    """
...
def analyze_events(dep, fault_slip, tstart, options):
    """
    Analyze rupture length, average slip, and event class for detected events.
    ...
    fault_slip : array-like
        Slip accumulated on the fault during each event
    ...
    event_properties : tuple
        For earthquakes, rupture length, average slip, system-wide event indices, partial rupture indices, event clusters, and rupture subclass indices; for SSEs, rupture length and average slip.
    """
...
def cluster_events(tstart):
    ...
    Returns:
    --------
    event_cluster : numpy.ndarray
        Cluster label for each event.
    """
...
def compute_STF(save_dir, outputs, dep, cumslip_outputs):
    """
    Compute the source time function from cumulative slip outputs.
    ...
    --------
    t, Fdot : tuple
        Time array and source time function values.
    """
    from scipy import integrate, interpolate
...
def compute_M0(save_dir, rupture_length, av_slip, mode, Mw):
    """
    Compute seismic moment or moment magnitude.
    ...
    rupture_length : array-like
        Event rupture length array
    
    av_slip : array-like
        Average event slip array
    
    mode : str
        Moment calculation mode
    
    Mw : array-like
        Moment magnitude array
    
    Returns:
    --------
    M0_or_Mw : numpy.ndarray
        Seismic moment when mode is M0, or moment magnitude when mode is Mw.
    """
    params = np.load('%s/const_params.npy'%(save_dir), allow_pickle=True)
    if 'DZ' in save_dir:
        mu = params.item().get('mu_damage')*1e9
...
def compute_GR(save_dir, cumslip_outputs, spin_up_idx, cutoff_Mw, npts):
    """
    Compute Gutenberg-Richter frequency-magnitude statistics.
    ...
    spin_up_idx : int
        Index of the first event after spin-up
    
    cutoff_Mw : float
        Minimum magnitude used for fitting
    ...
    Returns:
    --------
    baseval, N, b, x, yN, a : tuple
        Magnitude-bin centers, cumulative event counts, b-value, fitted magnitudes, fitted log-counts, and intercept.
    """
