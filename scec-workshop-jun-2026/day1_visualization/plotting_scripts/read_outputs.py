...
def base_event_criteria():
    """
    Create default event-detection criteria.
    ...
    --------
    options : dict
        Default event-detection options dictionary.
    """
    options = {
        'mode' : 'sliprate',
        'Pths_mult' : 1e-2,
...
def read_fault_probe_outputs(save_dir, options):
    """
    Read Tandem fault probe outputs and depth information.
    
    Parameters:
    -----------
    save_dir : str
        Directory containing Tandem simulation fault probe outputs
    
    options : dict
        Reading/writing outputs options
    
    Returns:
    --------
    outputs, dep : tuple
        Fault probe output array and corresponding depth array.
    """
    import time
...
def read_domain_probe_outputs(save_dir, options):
    """
    Read Tandem domain probe outputs and station locations.
    
    Parameters:
    -----------
    save_dir : str
        Directory containing Tandem simulation domain probe outputs
    ...
    --------
    outputs, xyloc : tuple
        domain probe output array and station coordinate array.
    """
...
def get_event_info(outputs, dep, **kwargs):
    """
    Load or compute earthquake information.
    ...
    **kwargs : dict
    ...
    --------
    event_info : dict
        Earthquake information dictionary loaded from disk or computed from outputs.
    """
...
def read_pvd(fname):
    """
    Read time steps from a PVD file.
    ...
    --------
    time : numpy.ndarray
        Time values listed in the PVD file.
    """
...
def fault_output_idx_lab(target_var):
    """
    Get the column index and label for fault probe outputs.
    ...
    --------
    idx, label : tuple
        Column index and axis label for the selected fault probe outputs.
    """
...
def domain_output_idx_lab(target_var):
    """
    Get the column index and label for domain probe outputs.
    ...
    --------
    idx, label : tuple
        Column index and axis label for the selected domain probe outputs.
    """
...
class OUTPUTS:
    ...
    def get_outputs(self, **kwargs):
        """
        Read fault probe outputs and store common variables on the object.
    ...
    def get_domain_outputs(self, **kwargs):
        """
        Read domain probe outputs and store common variables on the object.
    ...
    def get_event_info(self, **kwargs):
        ...
        **kwargs : dict
        ...
        --------
        event_info : dict
            Earthquake information dictionary stored on the object.
        """
    ...
    def read_fp_csv(self, csvname, target_var):
        """
        Read one fault probe CSV file and store it on the object.
        ...
        stloc = pd.read_csv(fname, nrows=1, header=None)
        self.fp_dep = float(stloc.values[0][-1].split(']')[0])
        self.fp_time = outputs[:, 0]
        idx, self.fp_lab = fault_output_idx_lab(target_var)
        self.fp_data = outputs[:, idx]

    # ----- Plots
    def timeseries(self, target_var, **kwargs):
        """
        Plot a time series for a selected Tandem output variable.
